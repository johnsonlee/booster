...
import com.didiglobal.booster.gradle.getRuntimeClasspath
...
class GraphVariantProcessor : VariantProcessor {
    ...
    override fun process(variant: BaseVariant) {
        ...
        project.tasks.register("transitiveUpstreamProjects${variant.name.capitalize()}") {
            it.doFirst {
                println("#### ${project}:${variant.name}")
                project.getUpstreamProjects(true, variant).forEach { node ->
                    println("  💡 project ${node.path}")
                }
            }
        }
        project.tasks.register("noneTransitiveUpstreamProjects${variant.name.capitalize()}") {
            it.doFirst {
                println("#### ${project}:${variant.name}")
                project.getUpstreamProjects(false, variant).forEach { node ->
                    println("  💡 project ${node.path}")
                }
            }
        }
...
private fun BaseVariant.generateProjectGraph() {
    ...
    while (stack.isNotEmpty()) {
        val from = stack.pop()
        rootProject.project(from.path).getUpstreamProjects(false, this).filter { to ->
...
private fun Project.getUpstreamProjects(transitive: Boolean, variant: BaseVariant): Set<ProjectNode> {
    return getRuntimeClasspath(transitive, variant).map {
