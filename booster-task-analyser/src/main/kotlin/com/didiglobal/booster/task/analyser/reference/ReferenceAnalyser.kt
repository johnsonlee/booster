...
class ReferenceAnalyser(
    ...
    private val upstreamClassSets: Map<Artifact, Map<String, ClassSet<ClassNode, AsmClassFileParser>>> by lazy {
        project.upstreamArtifacts.associateWith {
    ...
    private val upstreamClassToVariant: Map<String, String> by lazy {
        upstreamClassSets.values.map { upstream ->
    ...
    private fun analyse(types: Iterable<Type>, edge: (ReferenceNode) -> Graph.Builder<ReferenceNode>) {
        types.filter {
            ...
            findReference(it.internalName)?.let { (artifact, _) ->
                edge(ReferenceNode(artifact.id.componentIdentifier.displayName, upstreamClassToVariant[it.internalName] ?: "", it.internalName))
            }
        }
    }
