...
internal class ArchivedClassSet<ClassFile, ClassParser : ClassFileParser<ClassFile>>(
    ...
    override fun load(): ArchivedClassSet<ClassFile, ClassParser> = once {
        val t0 = threadBean.currentThreadCpuTime
        val size = classes.size
        val t1 = threadBean.currentThreadCpuTime
        println("Load ${green(size)} classes from $location in ${Duration.ofNanos(t1 - t0).toMillis()}ms")
        this
    }
