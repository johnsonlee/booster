...
fun Project.getJarTaskProviders(variant: Variant? = null): Collection<TaskProvider<out Task>> = when {
    isAndroid -> when (getAndroidOrNull<BaseExtension>()) {
        is LibraryExtension -> filterByVariant(variant).mapNotNull(Variant::createFullJarTaskProvider)
        is AppExtension -> filterByVariant(variant).mapNotNull(Variant::bundleClassesTaskProvider)
        else -> emptyList()
    }
