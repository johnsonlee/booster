...
fun Project.getRuntimeClasspath(
        transitive: Boolean = true,
        variant: BaseVariant? = null
): Set<ResolvedArtifact> = when {
    isAndroid -> getRuntimeClasspath(transitive) { p ->
        p.filterByVariant(variant).map { v ->
            v.runtimeConfiguration
        }
    }
    isJava -> getRuntimeClasspath(transitive) { p ->
        listOf(p.configurations.getByName(RUNTIME_CLASSPATH_CONFIGURATION_NAME))
    }
    else -> emptySet()
}.distinctBy {
...
private fun Project.getRuntimeClasspath(transitive: Boolean, configure: (Project) -> List<Configuration>): Set<ResolvedArtifact> {
    val stack = Stack<Project>()
    val results = mutableSetOf<ResolvedArtifact>()

    stack.add(this)

    while (stack.isNotEmpty()) {
        val resolved = configure(stack.pop()).map { it.resolvedConfiguration.resolvedArtifacts }.flatten()
