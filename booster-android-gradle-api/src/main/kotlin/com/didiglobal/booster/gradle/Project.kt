...
): Collection<ResolvedArtifactResult> = when {
    isAndroid -> {
        filterByVariant(variant).map {
            it.getResolvedArtifactResults(transitive)
        }.flatten()
    }
    isJava -> getResolvedArtifactResults(transitive)
    else -> emptyList()
}.distinctBy {
    it.id.componentIdentifier
}

private fun Project.getResolvedArtifactResults(transitive: Boolean): Set<ResolvedArtifactResult> {
    ...
    while (stack.isNotEmpty()) {
        val resolved = stack.pop().configurations.getByName(RUNTIME_CLASSPATH_CONFIGURATION_NAME).resolvedConfiguration.resolvedArtifacts.map {
