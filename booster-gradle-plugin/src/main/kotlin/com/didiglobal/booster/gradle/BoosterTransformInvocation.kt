...
internal class BoosterTransformInvocation(
    ...
    private val outputs: MutableList<File> by lazy(::CopyOnWriteArrayList)

    private val collectors: MutableList<Collector<*>> by lazy (::CopyOnWriteArrayList)

    /*
     * Preload transformers as List to fix NoSuchElementException caused by ServiceLoader in parallel mode
     */
    private val transformers: List<Transformer> by lazy {
        transform.parameter.transformers.map {
            try {
    ...
    override val bootClasspath: Collection<File> by lazy(delegate::bootClasspath)

    override val compileClasspath: Collection<File> by lazy(delegate::compileClasspath)

    override val runtimeClasspath: Collection<File> by lazy(delegate::runtimeClasspath)
    ...
    override val applicationId: String by lazy(delegate::applicationId)

    override val originalApplicationId: String by lazy(delegate::originalApplicationId)

    override val isDebuggable: Boolean by lazy (variant.buildType::isDebuggable)

    override val isDataBindingEnabled: Boolean by lazy (delegate::isDataBindingEnabled)
